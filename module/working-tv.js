console.log("Working TV Addon Generator");
...
function panic() {
    [...document.getElementsByClassName("childMessage")].forEach((element) => {
        element.textContent =
            "Something went wrong! Please contact the team on discord.";
...
async function converting({ target: { files } }, uploadId) {
    ...
    console.log(
        `%cID of file targetting: ${uploadId}`,
    ...
    if (ffmpeg === null) {
        ...
        ffmpeg.on("log", ({ message }) => {
            ...
            [...document.getElementsByClassName("childMessage")].forEach(
                (element) => {
        ...
        ffmpeg.on("progress", ({ progress }) => {
            [...document.getElementsByClassName("childProgress")].forEach(
                (element) => {
    ...
    try {
        ...
        audioSlotBlob.push({
            name:
                "Working TV RP/sounds/music/c" +
                uploadId.replace("upload", "") +
                ".ogg",
        ...
        audioElement.addEventListener("loadedmetadata", function () {
            console.log(
                `%cAudio Duration: ${audioElement.duration}`,
    ...
    await ffmpeg.exec([
        ...
        name,
        "-vf",
        "scale=250:trunc(ow/a/2)*2,pad=250:250:(ow-iw)/2:(oh-ih)/2:black,hue=s=2",
        "-r",
        fps,
        "-c:a",
        "copy",
    ...
    document.getElementById(uploadId + "video").src = URL.createObjectURL(
        new Blob([scaledVideo.buffer], { type: "video/mp4" })
    );
    await ffmpeg.exec([
        "-i",
        `${uploadId}scaledVideo.mp4`,
    ...
    console.log(
        `%cFrame: ${directoryContents.length - 2}  |||  Grid: ${Math.ceil(
            Math.sqrt(directoryContents.length - 2)
        )}`,
    ...
    await ffmpeg.exec([
        ...
        fps,
        "-i",
        `concat:${outputPattern}`,
        "-vf",
        `tile=layout=${optimizedGrid}x${optimizedGrid}`,
        `${uploadId}output.png`,
    ]);
    ...
    imgSlotBlob.push({
        name:
            "Working TV RP/textures/entity/c" +
            uploadId.replace("upload", "") +
            ".png",
        lastModified: new Date(),
        input: blob,
...
tosCheckbox.addEventListener("change", () => {
    if (tosCheckbox.checked) {
        ...
        tosLabel.innerHTML =
            'I agree to the <a href="/TOS" target="_blank">Term of service</a>';
    } else {
        downloadButton.setAttribute("disabled", "disabled");
        tosLabel.innerHTML =
            'I <span style="text-decoration: underline;">disagree</span> to the <a href="/TOS" target="_blank">Term of service</a>';
    }
});
function errorWarning(error) {
    document.getElementById(
        "debugMessage"
    ).innerHTML += `<br><br><span style="color:red; font-size:2vw"> ${error} </span>`;
}
window.downloadTestZip = async function downloadTestZip() {
    ...
    try {
        ...
        for (let i = 0; i < 5; i++) {
            fetch(BPELIST[i])
                ...
                .then((jsonData) => {
                    ...
                    jsonData["minecraft:entity"]["description"]["properties"][
                        "working_tv:channel_frame"
                    ]["range"] = [gridIndex, gridIndex + 1];
                    jsonData["minecraft:entity"]["description"]["properties"][
                        "working_tv:channel_frame"
                    ]["default"] = `${gridIndex}`;
                    jsonData["minecraft:entity"]["description"]["properties"][
                        "working_tv:info_frame"
                    ]["range"] = [frameIndex, frameIndex + 1];
                    jsonData["minecraft:entity"]["description"]["properties"][
                        "working_tv:info_frame"
                    ]["default"] = `${frameIndex}`;
        ...
        fetch(
            "https://dl.dropboxusercontent.com/scl/fi/gb4tlqk4q02dzo9yaa0n9/c1.entity.json?rlkey=3ypyddciocngjakzguvqgnai3&dl=0"
        )
            .then((response) => response.json())
            .then((jsonData) => {
                for (let i = 0; i < 5; i++) {
                    RPEList.push({
                        ...
                        lastModified: new Date(),
                        input: JSON.stringify(jsonData).replace(
                            /c1/g,
                            `c${i + 1}`
                        ),
                    });
        ...
        fetch(
            "https://dl.dropboxusercontent.com/scl/fi/k0bik4nbiwd4ffygy6zng/working_tv.animation.json?rlkey=e01twrh64reaj8zhzdpjdw7c4&dl=0"
        )
            .then((response) => response.json())
            .then((jsonData) => {
                BPAnim = {
                    ...
                    lastModified: new Date(),
                    input: JSON.stringify(jsonData)
                        .replace("123456", sortedArray[0][1])
                        .replace("234567", sortedArray[1][1])
                        .replace("345678", sortedArray[2][1])
                        .replace("456789", sortedArray[3][1])
                        .replace("567890", sortedArray[4][1]),
                };
            });
    } catch (error) {
        console.error(error);
    ...
    //
    const RPM = {
        name: "Working TV RP/materials/entity.material",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/55dgbqa46su5rbwr70nw9/entity.material?rlkey=4fsl30e91gaulk1e4tyrio7g8&dl=0 "
        ),
    };
    console.log("LOADING RPM");
    updateFetch("LOADING RPM");
    const RPTET = {
        name: "Working TV RP/textures/terrain_texture.json",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/32krfwkoplempq1el9lwx/terrain_texture.json?rlkey=fjpwgc42s6rwz6bi77oigyn89&dl=0"
        ),
    };
    console.log("LOADING RPTET");
    updateFetch("LOADING RPTET");
    const RPBG = {
        name: "Working TV RP/models/blocks/tv.geo.json",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/37c6t9k0ovt184xmrsdbh/tv.geo.json?rlkey=h9ca8ao0ph0dq5jphmpw4tgrf&dl=0"
        ),
    };
    console.log("LOADING RPBG");
    updateFetch("LOADING RPBG");
    const RPEG = {
        name: "Working TV RP/models/entity/tv_content.geo.json",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/qktt0s95a32e5tj953rsj/tv_content.geo.json?rlkey=2me24mkdifvrow5r26avylr3q&dl=0"
        ),
    };
    console.log("LOADING RPEG");
    updateFetch("LOADING RPEG");
    const RPRC = {
        name: "Working TV RP/render_controllers/tv_content.render_controllers.json",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/u1j4gjzizvnj933nrzyjb/tv_content.render_controllers.json?rlkey=s6g03lv8k6im2qn7gi0l1kar3&dl=0"
        ),
    };
    console.log("LOADING RPRC");
    updateFetch("LOADING RPRC");
    const RPSD = {
        name: "Working TV RP/sounds/sound_definitions.json",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/yi8hty4xojy3rhvcfvb7u/sound_definitions.json?rlkey=y2in5s8m6hdqh4zmmci0qhx97&dl=0"
        ),
    };
    console.log("LOADING RPSD");
    updateFetch("LOADING RPSD");
    const RPTXT = {
        name: "Working TV RP/texts/en_US.lang",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/5ozefz7g7ymqu6xs8m3lb/en_US.lang?rlkey=2uz8yvq8aaa9a9moxc8s4lql4&dl=0"
        ),
    };
    console.log("LOADING RPTXT");
    updateFetch("LOADING RPTXT");
    const RPTB = {
        name: "Working TV RP/textures/blocks/tv.png",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/tkmmsingmunh25zxwz8ot/tv.png?rlkey=kx51gkjg48ggtjv1fl9mmhlvz&dl=0"
        ),
    };
    ...
    try {
        ...
        for (let i = 0; i < 5; i++) {
            if (slotArray.includes(i)) {
                console.log(
                    "The img index of " + i + " is in the array! Skipping..."
                );
                continue;
            } else {
                let RPTEO = {
    ...
    const RBBJ = {
        name: "Working TV RP/blocks.json",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/4jh3qxh5ta920u1ntujh3/blocks.json?rlkey=i3h2e1v7wi5llv60ja2ihjjmu&st=avbl7alm&dl=0"
        ),
    };
    console.log("LOADING RBBJ");
    updateFetch("LOADING RBBJ");
    const RPMF = {
        name: "Working TV RP/manifest.json",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/3q04qmkl0yeieuaiuy4lr/manifest.json?rlkey=ss0skvrljohfiwqfa38itabqt&dl=0"
        ),
    };
    console.log("LOADING RPMF");
    updateFetch("LOADING RPMF");
    const RPPIC = {
        name: "Working TV RP/pack_icon.png",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/k0qz6s22agyynbxdem782/pack_icon.png?rlkey=bxf1vjncs7t0nezenzxt5f3dx&dl=0"
        ),
    };
    ...
    ///////BP/////////
    const BPPIC = {
        name: "Working TV BP/pack_icon.png",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/k0qz6s22agyynbxdem782/pack_icon.png?rlkey=bxf1vjncs7t0nezenzxt5f3dx&dl=0"
        ),
    ...
    const BPMF = {
        name: "Working TV BP/manifest.json",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/yvz0dntmd0stkkyj2ohxh/manifest.json?rlkey=0rx4q0d0p1y84arxju197o41l&dl=0"
        ),
    };
    console.log("LOADING BPMF");
    updateFetch("LOADING BPMF");
    const BPAC = {
        name: "Working TV BP/animation_controllers/tv_content.animation_controllers.json",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/b7r3jspnl1of0lk625hle/tv_content.animation_controllers.json?rlkey=zy7i5bw4e9g15zrb741w24txx&dl=0"
        ),
    };
    console.log("LOADING BPAC");
    updateFetch("LOADING BPAC");
    const BPB = {
        name: "Working TV BP/blocks/tv.block.json",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/y0ahd6q2lgl6i0h0kzv4i/tv.block.json?rlkey=vfda3i53oyafxwihdcxh1m7pb&dl=0"
        ),
    };
    console.log("LOADING BPB");
    updateFetch("LOADING BPB");
    const BPSCRIPT = {
        name: "Working TV BP/scripts/main.js",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/5glze297bzj1h3dlh7ph3/main.js?rlkey=70xna36qt6zco497t2famr153&st=z092fz1l&dl=0"
        ),
    };
    console.log("LOADING BPSCRIPT");
    updateFetch("LOADING BPSCRIPT");
    const BPR = {
        name: "Working TV BP/recipes/working_tv.recipe.json",
        lastModified: new Date(),
        input: await fetch(
            "https://dl.dropboxusercontent.com/scl/fi/fl0kh1vz4vmsv82b42vo7/working_tv.recipe.json?rlkey=n072i26yy7tznxd06cgpuqfwa&dl=0"
        ),
    };
